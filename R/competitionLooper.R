#' Test metrics across multiple competitive simulation arenas
#'
#' Large, somewhat sloppy function tying many previous functions together into a single
#' competitive exclusion simulator function that generates spatial arenas, samples
...
#' @param competition.randomizations The number of generations per competitive exclusion
#' arena simulation
...
competitionLooper <- function(no.species, x.min, x.max, y.min, y.max, no.quadrats, quadrat_size, mean.log.individuals, max.distance, percent.killed, competition.randomizations, null.method, concatBYrichness=TRUE, no.randomizations, expectation, wrong, no.metrics, iterations, temp.file, output.file)
{
	...
	for(i in 1:iterations)
	{
		...
		arena <- randomArena(tree, x.min, x.max, y.min, y.max, mean.log.individuals)

		positions <- competitionSimulator(tree, arena, max.distance, percent.killed, competition.randomizations)
